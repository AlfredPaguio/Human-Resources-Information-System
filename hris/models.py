...
class Users(db.Model, UserMixin):
    ...
    employee_id = db.Column(
        db.Integer(), db.ForeignKey("employee_info.id", ondelete="CASCADE")
    )

    # Relationship
    announcement_info = db.relationship(
        "Announcements", backref="announcements", lazy=True
    )

    @hybrid_property
    def getAnnouncement(self):
        return self.announcement_info if self.announcement_info else None
...
class EmployeeInfo(db.Model):
    ...
    phil_health = db.Column(
        db.String(length=50), nullable=True, unique=False
    )  # Temporary
    ...
    # Relationship
    user_info = db.relationship("Users", backref="employee", uselist=False)
    employment_info = db.relationship("EmploymentInfo", backref="employee", uselist=False)
    attendance_info = db.relationship("Attendance", backref="employee", uselist=False)
    leave_info = db.relationship("Leave", backref="employee", uselist=False)
    payslip_info = db.relationship("Payslips", backref="employee", uselist=False)
    ...
    def getEmployment(self):
        return self.employment_info if self.employment_info else None

    @hybrid_property
    def getUserInfo(self):
        return self.user_info if self.user_info else None

    @hybrid_property
    def getAttendanceInfo(self):
        return self.attendance_info if self.attendance_info else None

    @hybrid_property
    def getLeaveInfo(self):
        return self.leave_info if self.leave_info else None
...
class Payslips(db.Model):
    ...
    status = db.Column(
        db.Enum(STATUS_TYPES), default=STATUS_TYPES.Pending, nullable=False
    )
...
class Attendance(db.Model):
    ...
    attendance_type = db.Column(
        db.Enum(ATTENDANCE_TYPES), default=ATTENDANCE_TYPES.Unavailable, nullable=False
    )
    status = db.Column(
        db.Enum(STATUS_TYPES), default=STATUS_TYPES.Pending, nullable=False
    )
        ...
        if self.pre_ot is not None or self.post_ot is not None:
            ...
            if self.checked_out is not None and self.checked_in is not None:
                ...
                if self.pre_ot is not None:
                    pre_ot_hours = (
                        datetime.combine(datetime.min, self.pre_ot)
                        - datetime.combine(datetime.min, self.start_shift)
                ...
                if self.post_ot is not None:
                    post_ot_hours = (
                        datetime.combine(datetime.min, self.post_ot)
                        - datetime.combine(datetime.min, self.end_shift)
                    ).total_seconds() / 3600.0
...
class Leave(db.Model):
    ...
    type = db.Column(
        db.Enum(LEAVE_TYPES), default=LEAVE_TYPES.Sick_Leave, nullable=False
    )
...
class Positions(db.Model):
    ...
    position_status = db.Column(
        db.Enum(POSITION_STATUS), default=POSITION_STATUS.Hiring, nullable=False
    )
    date_created = db.Column(db.Date(), default=func.current_date(), nullable=False)
    ...
    def get_department(self):
        return self.department_info if self.department_info else None
...
class Departments(db.Model):
    ...
    def getPositions(self):
        return self.positions_info if self.positions_info else None
